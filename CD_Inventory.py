...
# NKnight, 2022-Mar-27, updated program to handle track functionality
#------------------------------------------#
    ...
    if strChoice == 'l':
        ...
        while True:
            ...
            if strChoice == 'x':
                ...
                tplTrack_Info = IO.ScreenIO.get_track_info()
                PC.DataProcessor.add_track(tplTrack_Info, cd)
                IO.ScreenIO.show_tracks(cd)
                continue
            elif strChoice == 'r':
                trk_id = int(input('Enter track ID to delete'))
                DC.CD.rmv_track(trk_id)
                IO.ScreenIO.show_tracks(cd) 
                continue
            elif strChoice == 'd':
                IO.ScreenIO.show_tracks(cd)
